...
Server side routing will only request the data that’s needed, where as client side routing is handled or routed internally.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

PUT can map to both Create and Update depending on the existence of the URI used with the PUT.
POST can map to Update although it's typically used for Create. 
POST can also be a partial update so we don't need the proposed PATCH method.
